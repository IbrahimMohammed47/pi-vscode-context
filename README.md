...
- `vscode_context` returns the selection, bounded visible cursor context, or the complete active in-memory document up to 48KB.
- `vscode_diagnostics` returns current VS Code diagnostics for the active document or matched workspace roots.
- Results are compact, bounded, and preserve unsaved editor state.
- Positions are zero-based and range ends are exclusive.

Each VS Code window exposes an authenticated HTTP server on a random `127.0.0.1` port. Private discovery records under `~/.pi-vscode-context/instances` let Pi select the best matching window by workspace root and focus recency. Discovery records never contain editor contents or diagnostics.
