...
const {
  defaultDiscoveryDir: clientDiscoveryDir,
} = require("../pi-vscode-context/client");
...
test("server rejects requests without discovery bearer token", async () => {
  ...
  await assert.rejects(readFile(service.discoveryFile, "utf8"), {
    code: "ENOENT",
  });
});
...
test("server routes authenticated context, diagnostics, and browser-selection requests", async () => {
  ...
  const service = await startServer({
    ...
    getBrowserSelection: async () => ({
      selectedElement: { selector: "button#save" },
    }),
  });
  ...
  try {
    ...
    const contextResponse = await fetch(`${record.endpoint}/context`, {
      headers,
    });
    ...
    const diagnosticsResponse = await fetch(
      `${record.endpoint}/diagnostics?scope=workspace`,
      { headers },
    );
    ...
    const browserResponse = await fetch(
      `${record.endpoint}/browser-selection`,
      { headers },
    );
    assert.equal(browserResponse.status, 200);
    assert.deepEqual(await browserResponse.json(), {
      selectedElement: { selector: "button#save" },
    });
  } finally {
    await service.stop();
...
test("workspace-folder updates rewrite discovery state", async () => {
  ...
  try {
    await service.updateWorkspaceFolders([
      "/workspace/new",
      "/workspace/other",
    ]);
    const record = JSON.parse(await readFile(service.discoveryFile, "utf8"));
    assert.deepEqual(record.workspaceFolders, [
      "/workspace/new",
      "/workspace/other",
    ]);
  } finally {
