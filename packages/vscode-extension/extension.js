...
const {
  CAPTURE_COMMAND,
  createBrowserSelectionCapture,
} = require("./browserSelection");
...
async function activate(context) {
  if (vscode.env.remoteName && vscode.env.remoteName !== "ssh-remote") {
    vscode.window.showWarningMessage(
      "Pi VS Code Context supports local desktop and Remote SSH workspaces only.",
    );
    return;
  }

  const getEditor = trackEditor(vscode.window, context.subscriptions);
  const browserSelection = createBrowserSelectionCapture(
    vscode,
  ...
  try {
    service = await startServer({
      ...
      getDiagnostics: (scope) =>
        buildDiagnostics(vscode, getEditor(), scope, workspaceFolders),
    ...
    vscode.window.showErrorMessage(
      "Pi VS Code Context could not start its local context server.",
    );
  }
}
