...
function workspaceScore(record, cwd) {
  ...
  for (const folder of record.workspaceFolders) {
    ...
    if (child === '' || (!child.startsWith('..') && !isAbsolute(child))) score = Math.max(score, root.length);
  }
  return score;
}
...
function validRecord(record) {
  if (!record || record.version !== 1 || typeof record.token !== 'string') return false;
  try {
...
async function requestVSCode({
  ...
  if (signal?.aborted) throw new Error('VS Code request was cancelled.');
  if (records.length === 0) {
    throw new Error(`No VS Code server found for Pi cwd: ${cwd}. Open this cwd in local VS Code with Pi VS Code Context enabled, then retry.`);
  }
  ...
  if (signal?.aborted) throw new Error('VS Code request was cancelled.');
  if (timedOut) {
    throw new Error(`VS Code request timed out after ${timeoutMs}ms. Retry once; if it persists, reload the matching VS Code window.`);
  }
  throw new Error(`VS Code connection for Pi cwd ${cwd} is stale or unavailable. Reload the matching VS Code window, then retry.`);
}
