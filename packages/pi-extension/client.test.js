...
test('discovery selects the most specific server matching Pi cwd', async () => {
  ...
  await record(directory, 'root', ['/workspace'], undefined, { focused: true, lastFocusedAt: 300 });
  await record(directory, 'nested', ['/workspace/project'], undefined, { focused: false, lastFocusedAt: 100 });
  await record(directory, 'other', ['/other']);

  const matches = await findDiscovery('/workspace/project/src', directory);
  assert.deepEqual(matches.map(({ id }) => id), ['nested', 'root']);
});
...
test('discovery prefers focused then most recently focused matching window', async () => {
  ...
  await record(directory, 'old', ['/workspace'], undefined, { focused: false, lastFocusedAt: 100 });
  await record(directory, 'recent', ['/workspace'], undefined, { focused: false, lastFocusedAt: 300 });
  await record(directory, 'focused', ['/workspace'], undefined, { focused: true, lastFocusedAt: 200 });

  const matches = await findDiscovery('/workspace/src', directory);
  assert.deepEqual(matches.map(({ id }) => id), ['focused', 'recent', 'old']);
});
...
test('stale preferred window falls back to next live match', async () => {
  ...
  await record(directory, 'fallback', ['/workspace'], 'http://127.0.0.1:10002', { lastFocusedAt: 100 });
  await record(directory, 'preferred', ['/workspace'], 'http://127.0.0.1:10001', { focused: true, lastFocusedAt: 200 });
  const requests = [];
